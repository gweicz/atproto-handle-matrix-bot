...
import { GoogleAuth, DNS } from "https://googleapis.deno.dev/v1/dns:v1.ts";
...
async function setHandle(name, user, did, replyTo = null) {
    ...
    if (db.handles[handle]) {
        ...
        if (found.user === user) {
            if (found.did === did) {
                return sendMessage(room, `Tento handle (${handle}) už máte nastavený!`, replyTo)
            }
        } else {
            return sendMessage(room, `Tento handle (${handle}) je už bohužel obsazený.`, replyTo)
        }
    }

    const recs = await dns.resourceRecordSetsList(env.GCLOUD_DNS_ZONE, env.GCLOUD_PROJECT)
    if (recs.rrsets.find(r => r.name === handle + '.')) {
        return sendMessage(room, `Bohužel, tento handle (${handle}) si nastavit nemůžete :(`, replyTo)
    }

    let prevHandle = Object.keys(db.handles).find(h => db.handles[h].user === user)
    if (prevHandle) {
        const prevDomain = `_atproto.${prevHandle}.`
        console.log(prevDomain)
        const previous = recs.rrsets.find(r => r.name === prevDomain && r.type === "TXT")
        if (previous) {
            const dresp = await dns.resourceRecordSetsDelete(env.GCLOUD_DNS_ZONE, prevDomain, env.GCLOUD_PROJECT, "TXT")
            console.log(dresp)
        }
    ...
    const out = await dns.resourceRecordSetsCreate(env.GCLOUD_DNS_ZONE, env.GCLOUD_PROJECT, {
        name: subdomain,
        type: 'TXT',
        rrdatas: [
            `"did=${did}"`
    ...
    save()
    return sendMessage(room, `Hotovo! Váš handle je nastaven (${handle})! Nyní můžete verifikovat změnu handle na Bluesky.`, replyTo)
}
...
matrix.on("Room.timeline", async event => {
    ...
    if (type == "m.room.message"&& event.getRoomId() === room) {
        ...
        if (body.startsWith('!handle')) {
            const handleMatch = body.match(/^!handle ([a-zA-Z0-9-\.]+) (did=|)(did:plc:[a-z0-9]+)$/)
            if (handleMatch) {
                await setHandle(handleMatch[1], event.getSender(), handleMatch[3], event.getId())
            } else {
                await sendMessage(room, "Nesprávný příkaz! Zadávejte ve formě: \"!handle jméno did\", například: \"!handle tree did:plc:524tuhdhh3m7li5gycdn6boe\"", event.getId())
            }
